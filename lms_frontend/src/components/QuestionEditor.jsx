...
const QuestionEditor = ({ question, index = 0, isExpanded: controlledExpanded, onExpandedChange, onUpdated, onDeleted }) => {
  ...
  return (
    <div className="bg-white rounded-xl border-2 border-slate-200 shadow-sm overflow-hidden hover:shadow-md transition dark:bg-slate-800 dark:border-slate-700">
      {/* Header Row - Always Visible - Coursera Style */}
      <button
        type="button"
        className="w-full flex items-center justify-between px-4 py-3 text-left hover:bg-slate-50 transition-colors dark:hover:bg-slate-700"
        onClick={() => {
      ...
      >
        <div className="flex flex-col flex-1 min-w-0">
          ...
          <span className="text-xs text-slate-500 line-clamp-1 mt-0.5 dark:text-slate-400">
        ...
        <div className="flex items-center gap-3 ml-3">
          ...
          <svg
            className={`w-4 h-4 text-slate-400 transition-transform dark:text-slate-500 ${expanded ? 'rotate-180' : ''}`}
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
      ...
      {expanded && (
        <div 
          className="px-4 pb-4 pt-2 space-y-3 border-t border-slate-100 dark:border-slate-700"
          onClick={(e) => e.stopPropagation()}
        >
          {error && (
            <div className="text-xs text-red-600 bg-red-50 p-2 rounded dark:bg-red-900/30 dark:text-red-400">{error}</div>
          )}
          ...
          <div onClick={(e) => e.stopPropagation()}>
            ...
            <textarea
              ...
              rows={2}
              className="w-full px-3 py-2 text-sm border-2 border-slate-300 rounded-lg focus:outline-none focus:border-primary-500 focus:ring-2 focus:ring-primary-200 transition dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100"
              disabled={loading}
              onClick={(e) => e.stopPropagation()}
            />
          </div>
          ...
          >
            <div>
              ...
              <input
                ...
                min="1"
                className="w-16 px-2 py-1.5 text-sm border-2 border-slate-300 rounded-lg focus:outline-none focus:border-primary-500 focus:ring-2 focus:ring-primary-200 transition dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100"
                disabled={loading}
                onClick={(e) => e.stopPropagation()}
              />
            </div>
          ...
          {/* Choices Section */}
          <div 
            className="border-t border-slate-100 pt-3 dark:border-slate-700"
            onClick={(e) => e.stopPropagation()}
          ...
          >
            <div 
              ...
              onClick={(e) => e.stopPropagation()}
            >
              <h4 className="text-sm font-semibold text-slate-900 dark:text-slate-100">Choices</h4>
            </div>
            ...
            >
              <input
                ...
                className="flex-1 px-3 py-1.5 text-sm border-2 border-slate-300 rounded-lg focus:outline-none focus:border-primary-500 focus:ring-2 focus:ring-primary-200 transition dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100"
                onKeyPress={(e) => {
                  if (e.key === 'Enter') {
                    e.stopPropagation();
                    handleAddChoice();
