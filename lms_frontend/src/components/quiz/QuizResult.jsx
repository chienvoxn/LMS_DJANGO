...
const QuizResult = ({
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-white via-slate-50 to-white dark:from-slate-900 dark:via-slate-900 dark:to-slate-900">
      ...
      <div className="sticky top-0 z-10 bg-white border-b-2 border-slate-200 shadow-md dark:bg-slate-800 dark:border-slate-700">
        <div className="max-w-5xl mx-auto px-6 py-5">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <button
                onClick={() => navigate(-1)}
                className="px-5 py-2.5 rounded-lg font-semibold bg-slate-200 hover:bg-slate-300 text-slate-700 transition-all border-2 border-slate-300 dark:bg-slate-700 dark:hover:bg-slate-600 dark:text-slate-300 dark:border-slate-600"
              >
                Back to Course
              </button>
              {onRetake && (
      ...
      <div className="max-w-5xl mx-auto px-6 py-10">
        ...
        <div className="bg-gradient-to-br from-primary-50 via-secondary-50 to-accent-50 rounded-2xl p-10 border-2 border-primary-200 text-center shadow-lg mb-8">
          <h2 className="text-3xl font-bold text-slate-900 mb-6 dark:text-slate-100">{quiz?.title || 'Quiz Results'}</h2>
          <div className="flex items-center justify-center gap-12 mb-8">
            <div className="bg-white rounded-2xl p-6 shadow-md border-2 border-primary-200 dark:bg-slate-800">
              ...
              <div className="text-base font-semibold text-slate-600 dark:text-slate-400">Score</div>
            </div>
            <div className="w-px h-20 bg-slate-300 dark:bg-slate-600"></div>
            <div className="bg-white rounded-2xl p-6 shadow-md border-2 border-primary-200 dark:bg-slate-800">
              <div className="text-4xl font-bold text-slate-900 mb-2 dark:text-slate-100">
                {result?.score || 0} / {result?.total_points || 0}
              </div>
              <div className="text-base font-semibold text-slate-600 dark:text-slate-400">Points</div>
            </div>
          </div>
          ...
          {questions.length > 0 && (
            <div className="flex items-center justify-center gap-8">
              <div className="flex items-center gap-3 bg-accent-50 px-6 py-3 rounded-full border-2 border-accent-200">
                ...
                <span className="text-slate-900 font-bold dark:text-slate-100">
