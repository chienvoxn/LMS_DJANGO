...
const QuizPlayer = ({
  ...
  return (
    <div className="flex h-full bg-gradient-to-br from-white via-slate-50 to-primary-50 dark:from-slate-900 dark:via-slate-900 dark:to-slate-800">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="bg-white border-b-2 border-slate-200 px-8 py-5 shadow-sm dark:bg-slate-800 dark:border-slate-700">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-2xl font-bold text-slate-900 mb-2 dark:text-slate-100">{quiz?.title}</h2>
        ...
        <div className="flex-1 overflow-y-auto p-8">
          <div className="max-w-4xl mx-auto">
            ...
            <div className="bg-white rounded-2xl border-2 border-slate-200 p-8 mb-8 shadow-lg dark:bg-slate-800 dark:border-slate-700">
              <div className="flex items-start justify-between mb-6">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <span className="px-4 py-2 bg-slate-100 text-slate-700 rounded-lg text-sm font-semibold border-2 border-slate-200 dark:bg-slate-700 dark:text-slate-300 dark:border-slate-700">
                      {currentQuestion.points} {currentQuestion.points === 1 ? 'point' : 'points'}
                    </span>
                  </div>
                  <p className="text-xl font-bold text-slate-900 mb-8 leading-relaxed dark:text-slate-100">
                    ...
                    <label
                      ...
                      className={`flex items-start gap-4 p-5 rounded-xl border-2 cursor-pointer transition-all ${
                        isSelected
                          ...
                          : 'border-slate-200 hover:border-primary-300 hover:bg-slate-50 dark:border-slate-700 dark:hover:bg-slate-700'
                      }`}
                    >
                      ...
                      <span className={`flex-1 text-base font-medium ${
                        isSelected ? 'text-primary-900' : 'text-slate-900 dark:text-slate-100'
                      }`}>
                        {choice.text}
                      </span>
                    </label>
        ...
        <div className="bg-white border-t-2 border-slate-200 px-8 py-5 shadow-md dark:bg-slate-800 dark:border-slate-700">
          <div className="max-w-4xl mx-auto flex items-center justify-between">
            <button
              onClick={handlePrevious}
              disabled={currentQuestionIndex === 0}
              className="px-8 py-3 rounded-lg font-bold bg-slate-200 hover:bg-slate-300 text-slate-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-sm hover:shadow-md border-2 border-slate-300 dark:bg-slate-700 dark:hover:bg-slate-600 dark:text-slate-300 dark:border-slate-600"
            >
              ← Previous
            </button>

            <div className="flex items-center gap-4">
              <div className="px-5 py-2 bg-slate-100 rounded-lg border-2 border-slate-200 dark:bg-slate-700 dark:border-slate-700">
      ...
      {showSubmitModal && (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl p-8 max-w-lg w-full mx-4 shadow-2xl border-2 border-slate-200 dark:bg-slate-800 dark:border-slate-700">
            <div className="w-16 h-16 bg-amber-100 rounded-full flex items-center justify-center mx-auto mb-4">
              <svg className="w-8 h-8 text-amber-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
              </svg>
            </div>
            <h3 className="text-2xl font-bold text-slate-900 mb-3 text-center dark:text-slate-100">Submit Quiz?</h3>
            <p className="text-base text-slate-600 mb-8 text-center leading-relaxed dark:text-slate-400">
            ...
            <div className="flex items-center gap-4">
              <button
                ...
                className="flex-1 px-6 py-3 rounded-lg font-bold bg-slate-200 hover:bg-slate-300 text-slate-700 transition-all shadow-sm hover:shadow-md border-2 border-slate-300 dark:bg-slate-700 dark:hover:bg-slate-600 dark:text-slate-300 dark:border-slate-600"
              >
                Cancel
              </button>
              <button
