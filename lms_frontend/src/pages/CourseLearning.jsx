...
const CourseLearning = () => {
  ...
  if (error) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-900 transition-colors">
        <div className="max-w-md bg-white dark:bg-slate-800 rounded-xl shadow-lg p-6">
          <h2 className="text-xl font-bold text-red-600 dark:text-red-400 mb-2">Error</h2>
          <p className="text-slate-700 dark:text-slate-300 mb-4">{error}</p>
          <button
            onClick={() => navigate(`/courses/${courseId}`)}
            className="w-full bg-sky-600 hover:bg-sky-700 dark:bg-sky-500 dark:hover:bg-sky-600 text-white px-4 py-2 rounded-lg transition"
          >
            Back to Course
          </button>
        </div>
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 flex flex-col transition-colors">
      ...
      <div className="bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          >
            {sidebarOpen ? (
              <svg className="w-6 h-6 text-slate-600 dark:text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 19l-7-7 7-7M14 5v14" />
              </svg>
            ) : (
              <svg className="w-6 h-6 text-slate-600 dark:text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <h1 className="text-lg font-bold text-slate-900 dark:text-slate-100">{course?.title || 'Course'}</h1>
        </div>
        <button
          onClick={() => navigate(`/courses/${courseId}`)}
          className="text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-100 p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg transition"
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div
          ...
          } fixed lg:static inset-y-0 left-0 z-30 w-80 bg-white dark:bg-slate-800 border-r border-slate-200 dark:border-slate-700 overflow-hidden transition-all duration-300 ${
          ...
          {currentLesson ? (
            <div className="max-w-4xl mx-auto p-4 lg:p-8 space-y-6">
              ...
              {user?.role === 'student' && enrollmentType === 'paid' && isCourseCompleted && (
                <div className="bg-accent-50 dark:bg-accent-900/20 border border-accent-200 dark:border-accent-800 p-4 rounded-xl flex items-center justify-between">
                  <div>
                    <p className="font-semibold text-accent-800 dark:text-accent-300">Course completed</p>
                    <p className="text-sm text-accent-700 dark:text-accent-400">You can now get your certificate of completion.</p>
                  </div>
                  <button
              ...
              <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-6 lg:p-8 shadow-sm">
                <h1 className="text-2xl lg:text-3xl font-bold text-slate-900 dark:text-slate-100 mb-3 leading-tight break-words">
                  {currentLesson.title}
                </h1>
                ...
                {currentLesson.content && (
                  <div className="prose prose-slate dark:prose-invert max-w-none mb-6">
                    <div className="text-slate-700 dark:text-slate-300 leading-relaxed space-y-4 text-justify">
                ...
                <div className="flex flex-col sm:flex-row gap-4 items-stretch sm:items-center pt-6 border-t border-slate-200 dark:border-slate-700">
                  <button
                    onClick={(e) => {
                      e.preventDefault();
                      e.stopPropagation();
                  ...
                  <div className="flex gap-2 sm:ml-auto">
                    {prevLesson && (
                      <button
                        ...
                        className="px-4 py-3 rounded-lg font-medium bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-300 transition flex-1 sm:flex-none"
                      >
                        ← Previous
                      </button>
                    )}
            ...
            <div className="flex items-center justify-center h-full">
              <p className="text-slate-600 dark:text-slate-400">Select a lesson to start learning</p>
            </div>
          )}
        </div>
      </div>
