...
{/* Header Section - Coursera Style */}
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
            <div>
              ...
              <p className="text-base text-slate-600 dark:text-slate-400">
                Welcome back, <span className="font-semibold text-primary-600 dark:text-primary-400">{user?.full_name || user?.email || 'Teacher'}</span>
              </p>
            </div>
            <button
          ...
          <div className="mb-6 bg-red-50 dark:bg-red-900/20 border-l-4 border-red-500 dark:border-red-400 rounded-lg p-4 shadow-sm">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="ml-4 text-sm font-semibold text-red-600 dark:text-red-400 hover:text-red-800 dark:hover:text-red-300 transition"
              >
                Dismiss
              </button>
            </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 lg:gap-6 mb-8">
          <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm px-6 py-5 hover:shadow-md transition-all">
            <div className="flex items-center justify-between mb-3">
              <div className="text-sm font-medium text-slate-600 dark:text-slate-400">Total Courses</div>
            ...
            <div className="text-3xl font-bold text-primary-500 dark:text-primary-400">{totalCourses}</div>
          </div>
          <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm px-6 py-5 hover:shadow-md transition-all">
            <div className="flex items-center justify-between mb-3">
              <div className="text-sm font-medium text-slate-600 dark:text-slate-400">Published Courses</div>
            ...
            <div className="text-3xl font-bold text-accent-500 dark:text-accent-400">{publishedCourses}</div>
          </div>
          <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm px-6 py-5 hover:shadow-md transition-all">
            <div className="flex items-center justify-between mb-3">
              <div className="text-sm font-medium text-slate-600 dark:text-slate-400">Draft Courses</div>
            ...
            <div className="text-3xl font-bold text-slate-700 dark:text-slate-300">{draftCourses}</div>
          </div>
        </div>
        ...
        <div>
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
            <div>
              <h2 className="text-2xl font-bold text-slate-900 dark:text-slate-100 mb-1">Your Courses</h2>
              <p className="text-sm text-slate-500 dark:text-slate-400">Manage and organize your course content</p>
            </div>
            {/* Search Bar - Coursera Style */}
            <div className="w-full sm:w-80 bg-white dark:bg-slate-800 rounded-lg border border-slate-300 dark:border-slate-600 px-4 py-2.5 flex items-center gap-3 focus-within:border-primary-500 dark:focus-within:border-primary-400 focus-within:ring-2 focus-within:ring-primary-100 dark:focus-within:ring-primary-900 transition-all shadow-sm">
              ...
              {searchQuery && (
                <button
                  ...
                  className="text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 transition p-1 rounded-full hover:bg-slate-100 dark:hover:bg-slate-700"
                >
          ...
          {/* Search Results Info */}
          {searchQuery && (
            <div className="mb-6 px-4 py-2.5 bg-primary-50 dark:bg-primary-900/20 border border-primary-200 dark:border-primary-800 rounded-lg text-sm text-primary-700 dark:text-primary-300">
          ...
          {courses.length === 0 ? (
            <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm p-12 lg:p-16 text-center">
              {searchQuery ? (
                <>
                  ...
                  <p className="text-slate-600 dark:text-slate-400 mb-6 max-w-md mx-auto">
                    No courses match your search "<span className="font-semibold text-slate-900 dark:text-slate-100">{searchQuery}</span>"
                  </p>
                  <button
                    onClick={() => setSearchQuery('')}
                    className="px-6 py-2.5 bg-slate-600 hover:bg-slate-700 dark:bg-slate-700 dark:hover:bg-slate-600 text-white rounded-lg font-medium transition shadow-sm hover:shadow-md"
                  >
                    Clear Search
                  </button>
                </>
