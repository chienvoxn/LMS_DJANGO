...
const CourseLandingPage = () => {
    ...
    try {
      ...
      const [
        ...
        reviewsAPI
          ...
          .catch(() => ({ data: { average_rating: 0, total_reviews: 0 } })),
      ...
      setRatingSummary(
        summaryResponse.data || { average_rating: 0, total_reviews: 0 },
      );
  ...
  const fetchQuizzes = async () => {
    try {
      ...
      const publishedQuizzes = quizzesData.filter(
      ...
      for (const quiz of publishedQuizzes) {
        try {
          const attemptResponse = await studentQuizAPI.getMyQuizAttempt(
            quiz.id,
          );
  ...
  const fetchAssignments = async () => {
    try {
      ...
      const response =
        await studentAssignmentAPI.getCourseAssignments(courseId);
      let assignmentsData = [];
      if (Array.isArray(response.data)) {
      ...
      for (const assignment of publishedAssignments) {
        try {
          const submissionResponse = await studentAssignmentAPI.getMySubmission(
            assignment.id,
          );
  ...
  const totalDuration =
    curriculum?.sections?.reduce((total, section) => {
      return (
        total +
            ...
            <div className="flex flex-wrap items-center gap-4 mb-6">
              <div className="flex items-center gap-2 bg-slate-50 dark:bg-slate-700 rounded-lg px-4 py-2">
                ...
                <span className="text-sm text-slate-600 dark:text-slate-400">
                  ...
                  {ratingSummary.total_reviews === 1 ? "rating" : "ratings"})
                </span>
              </div>
          ...
          {course.description && (
            <div className="bg-white dark:bg-slate-800 rounded-2xl p-8 border-2 border-slate-200 dark:border-slate-700 shadow-sm">
              ...
              <div className="prose max-w-none">
                <p className="text-lg text-slate-700 dark:text-slate-300 whitespace-pre-line leading-relaxed">
                  {course.description}
                </p>
              </div>
            </div>
              ...
              {instructor ? (
                <div className="flex items-start gap-6">
                  <div className="relative">
                    <img
                      src={
                        ...
                        `https://ui-avatars.com/api/?name=${encodeURIComponent(instructor.full_name || "Instructor")}&background=0056D2&color=fff&size=128`
                      }
                  ...
                  <div className="flex-1">
                      ...
                      <div className="flex flex-wrap items-center gap-6 text-sm mb-4">
                        {instructor.stats.average_rating > 0 && (
                          <div className="flex items-center gap-2 bg-slate-50 dark:bg-slate-700 rounded-lg px-3 py-2">
                            <RatingStars
                              value={instructor.stats.average_rating}
                              size="sm"
                            />
                            ...
                            <span className="text-slate-600 dark:text-slate-400">
                              ({instructor.stats.total_reviews || 0}{" "}
                              {instructor.stats.total_reviews === 1
                                ? "review"
                                : "reviews"}
                              )
                            </span>
                        ...
                        {instructor.stats.total_courses > 0 && (
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-semibold text-slate-700 dark:text-slate-300">
                              {instructor.stats.total_courses}{" "}
                              {instructor.stats.total_courses === 1
                                ? "course"
                                : "courses"}
                            </span>
                          </div>
                        ...
                        {instructor.stats.total_students > 0 && (
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-semibold text-slate-700 dark:text-slate-300">
                              {instructor.stats.total_students}{" "}
                              {instructor.stats.total_students === 1
                                ? "student"
                                : "students"}
                            </span>
                          </div>
                    ...
                    <button
                      onClick={() =>
                        navigate(`/instructor/${course.teacher_id}/profile`)
                      }
                ...
                <div className="space-y-4">
                  {quizzes.map((quiz) => {
                    ...
                    return (
                      <div
                        key={quiz.id}
                        className="bg-slate-50 dark:bg-slate-900 border-2 border-slate-200 dark:border-slate-800 rounded-xl p-6 hover:border-primary-200 dark:hover:border-primary-800 hover:shadow-md dark:hover:shadow-cyan-950/20 transition-all"
                      >
                        <div className="flex items-center justify-between gap-4">
                          <div className="flex-1">
                            ...
                            <div className="flex flex-wrap items-center gap-4 text-sm text-slate-600 dark:text-slate-400">
                              ...
                              {isCompleted && result && (
                                <div className="flex items-center gap-2 bg-accent-50 dark:bg-accent-950/30 px-3 py-1 rounded-full border border-accent-200 dark:border-accent-800">
                          ...
                          <button
                            ...
                            className="flex-shrink-0 px-6 py-3 bg-primary-500 hover:bg-primary-600 dark:bg-primary-600 dark:hover:bg-primary-500 text-white font-bold rounded-lg transition-all shadow-md hover:shadow-lg transform hover:-translate-y-0.5"
                          >
                            {isCompleted
                              ? "Review"
                              : status === "in_progress"
              ...
              {!loadingAssignments && assignments.length > 0 && (
                <div className="space-y-4">
                  {assignments.map((assignment) => {
                    const status =
                      assignmentStatuses[assignment.id] || "not_submitted";
                    ...
                    return (
                      <div
                        key={assignment.id}
                        className="bg-slate-50 dark:bg-slate-900 border-2 border-slate-200 dark:border-slate-800 rounded-xl p-6 hover:border-primary-200 dark:hover:border-primary-800 hover:shadow-md dark:hover:shadow-cyan-950/30 transition-all"
                      >
                        <div className="flex items-center justify-between gap-4">
                          <div className="flex-1">
                            <div className="flex items-center gap-3 mb-3">
