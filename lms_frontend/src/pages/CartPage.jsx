    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-white via-slate-50 to-white dark:from-slate-900 dark:via-slate-900 dark:to-slate-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-slate-600 dark:text-slate-400 font-medium">Loading cart...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-white via-slate-50 to-white dark:from-slate-900 dark:via-slate-900 dark:to-slate-900">
      <div className="max-w-7xl mx-auto px-6 py-10">
        ...
        <div className="mb-8">
          <h1 className="text-4xl md:text-5xl font-bold text-slate-900 dark:text-slate-100 mb-2">Shopping Cart</h1>
          <p className="text-lg text-slate-600 dark:text-slate-400">Review your selected courses and proceed to checkout</p>
        </div>
        ...
        <div className="grid gap-8 lg:grid-cols-[2fr,1fr]">
          ...
          <div>
            {items.length === 0 ? (
              <div className="bg-white dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 rounded-2xl p-16 text-center shadow-lg">
                <div className="w-32 h-32 bg-gradient-to-br from-primary-100 to-secondary-100 dark:from-slate-800 dark:to-slate-700 rounded-full flex items-center justify-center mx-auto mb-6">
              ...
              <div>
                ...
                <div className="mb-6 flex items-center gap-3 bg-white dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 rounded-xl p-4 shadow-sm">
                  <input
                    type="checkbox"
                    checked={selectedItems.size === items.length && items.length > 0}
                    onChange={handleSelectAll}
                    className="w-5 h-5 text-primary-500 border-slate-300 dark:border-slate-600 rounded focus:ring-primary-500 focus:ring-2 cursor-pointer"
                  />
                ...
                <div className="space-y-4">
                  {items.map((item) => (
                    <div
                      key={item.id}
                      className={`flex gap-4 bg-white dark:bg-slate-800 border-2 rounded-xl p-5 shadow-md transition-all duration-200 hover:shadow-lg ${
                    ...
                    >
                      <input
                        ...
                        className="w-5 h-5 text-primary-500 border-slate-300 dark:border-slate-600 rounded focus:ring-primary-500 focus:ring-2 mt-1 cursor-pointer flex-shrink-0"
                      />
                      ...
                      <div className="flex-1 min-w-0 flex flex-col justify-between">
                        <div>
                          <h3 className="font-bold text-slate-900 dark:text-slate-100 line-clamp-2 text-base mb-2">
                            {item.course_title}
                          </h3>
          ...
          <div>
            <div className="bg-white dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 rounded-2xl p-6 shadow-lg sticky top-4">
              <div className="flex items-center gap-2 mb-4">
                <svg className="w-6 h-6 text-primary-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                </svg>
                <p className="text-xl font-bold text-slate-900 dark:text-slate-100">Summary</p>
              </div>
              ...
              <div className="mt-4 pt-4 border-t-2 border-slate-200 dark:border-slate-700">
                <div className="flex items-center justify-between mb-2">
                  <p className="text-base font-medium text-slate-600 dark:text-slate-400">Total:</p>
              ...
              {items.length > 0 && (
                <button
                  onClick={() => navigate('/browse')}
                  className="w-full mt-3 rounded-lg border-2 border-primary-500 text-primary-600 py-3 font-semibold hover:bg-primary-50 dark:hover:bg-primary-900/20 transition-all"
                >
                  Continue Shopping
                </button>
              )}
